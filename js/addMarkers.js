  ...
  init: async function () {
    ...
    dishes.map(dish => {
      ...
      if (!dish.unavailable_days.includes(days[todaysDay])) {
        ...
        // Dish Rating plane
        dish_rating_plane=document.createElement('a-entity')
        dish_rating_plane.setAttribute('id',`rating-plane-${dish.id}`)
        dish_rating_plane.setAttribute('position',{x:2,y:0,z:0.5})
        dish_rating_plane.setAttribute('geometry',{primitive:'plane',width:2,height:0.4})
        dish_rating_plane.setAttribute('material',{color:'yellow'})
        dish_rating_plane.setAttribute('rotation',{x:0,y:0,z:0})
        dish_rating_plane.setAttribute('visible',false)

        // Ratings
        dish_rating=document.createElement('a-entity')
        dish_rating.setAttribute("id",`dish-rating-${dish.id}`)
        dish_rating.setAttribute("position",{x:0,y:0,z:0.6})
        dish_rating.setAttribute('text',{
      ...
      dish_rating_plane.appendChild(dish_rating)
      dish_rating_plane.appendChild(marker)
